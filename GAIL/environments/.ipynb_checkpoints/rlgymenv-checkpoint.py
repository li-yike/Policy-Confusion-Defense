...
class RLGymSim(policyopt.Simulation):
    def __init__(self, env_name):
        ...
        self.action_space = self.env.action_space # 记录动作空间大小
        self.curr_obs = self.env.reset() # 重置环境后的环境信息
        self.is_done = False # 是否到达终态
...
def _convert_space(space):
    ...
    if isinstance(space, spaces.Box):
        print("space.low.ndim=", space.low.ndim)
        print("space.low.shape=", space.low.shape)
...
class RLGymMDP(policyopt.MDP):
    def __init__(self, env_name):
        ...
        tmpsim = self.new_sim()
        # Converts a rl-gym space to our own space representation，把obs_space、action_space转换为自己定义的连续空间类
