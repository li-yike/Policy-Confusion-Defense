...
class Policy(nn.Model):
    def __init__(self, obsfeat_space, action_space, num_actiondist_params, enable_obsnorm, varscope_name):
        ...
        with nn.variable_scope(varscope_name) as self.__varscope:
            # Action distribution for this current policy 当前策略的动作分布
            ...
            normalized_obsfeat_B_Df = self.obsnorm.standardize_expr(obsfeat_B_Df) #将数据标准化
            # Convert (normalized) observations to action distributions，将输入的环境变量转换为动作分布概率值
        ...
        # 利用动作分布参数，计算动作分布概率
        logprobs_B = self._make_actiondist_logprob_ops(actiondist_B_Pa, input_actions_B_Da) # 得到动作高斯概率密度分布（对数形式）
        # Proposal distribution from old policy
        # 旧策略的动作分布参数，计算动作分布概率
        proposal_actiondist_B_Pa = tensor.matrix(name='proposal_actiondist_B_Pa')
        proposal_logprobs_B = self._make_actiondist_logprob_ops(proposal_actiondist_B_Pa, input_actions_B_Da) # 得到旧策略的动作高斯概率密度分布
        # Local RL objective
################################################################################################################################################################
        ...
        klgrad_P = thutil.flatgrad(kl, param_vars) # 对kl中的参数求梯度
        hvpexpr = thutil.flatgrad((klgrad_P*v_P).sum(), param_vars) # 二阶梯度
...
class GaussianPolicy(Policy):
    def __init__(self, cfg, obsfeat_space, action_space, varscope_name):
        ...
        assert isinstance(obsfeat_space, ContinuousSpace) and isinstance(action_space, ContinuousSpace) # 判断动作空间和状态空间是否均为连续空间
        ...
        Policy.__init__(
            ...
            num_actiondist_params=action_space.dim*2, # 动作分布参数的数量 = 动作空间维度的2倍
            enable_obsnorm=cfg.enable_obsnorm, # 是否需要对状态变量进行一般化
            varscope_name=varscope_name) 


    def _make_actiondist_ops(self, obsfeat_B_Df):
        # Computes action distribution mean (of a Gaussian) using MLP 计算动作概率分布的均值
        ...
        means_B_Da = mean_layer.output # 输出的是根据环境的动作概率平均值

        # Action distribution log standard deviations are parameters themselves 动作分布的log标准差就是参数本身 log std = e
        # 定义动作概率分log对数的标准差，后面也是要更新的参数
        # （init_logstedv,init_logstedv）
...
def compute_advantage(r, obsfeat, time, value_func, gamma, lam):
    ...
    # Time-dependent baseline that cheats on the current batch 未理解
    ...
    # State-dependent baseline (value function) 计算state-value
...
class ValueFunc(nn.Model):
    def __init__(self, hidden_spec, obsfeat_space, enable_obsnorm, enable_vnorm, varscope_name, max_kl, damping, time_scale):
        self.hidden_spec = hidden_spec # 隐藏层
        self.obsfeat_space = obsfeat_space # 环境空间
        self.enable_obsnorm = enable_obsnorm # 环境空间一般化
        self.enable_vnorm = enable_vnorm # 值函数一般化
        self.max_kl = max_kl # 最大KL散度
        self.damping = damping # 未知
        self.time_scale = time_scale # 时间衰减
        ...
        with nn.variable_scope(varscope_name) as self.__varscope:
            ...
            # Compute (normalized) value of states using a feedforward network 使用反馈网络计算状态值函数
            with nn.variable_scope('hidden'):
        ...
        param_vars = self.get_trainable_variables() # 获得可训练参数
