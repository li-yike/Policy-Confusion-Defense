...
def _hash_name2array(name2array):
    ...
    return hashlib.sha1(('|'.join('%s %s' for n, h in sorted([(name, hash_array(a)) for name, a in name2array]))).encode("utf8")).hexdigest()
...
class Model(object):
    ...
    def load_h5(self, h5file, key):
        with h5py.File(h5file, 'r') as f:
            ...
            assert h == str(dset.attrs['hash'], 'utf-8'), 'Checkpoint hash %s does not match loaded hash %s' % (dset.attrs['hash'], h)
    ...
    def __init__(self, input_B_Di, input_shape, output_shape, initializer):
        ...
        with variable_scope(type(self).__name__) as self.__varscope:
            if initializer is None:
                ...
                # Glorot/Bengio 2010
                # 如果初始值没有定义，就固定为6./(input_shape[0] + output_shape[0])，从中随机一个值作为w的初始值
            ...
            self.W_Di_Do = get_variable('W', initializer.astype(theano.config.floatX)) # 定义变量W，初始为float类型
            self.b_1_Do = get_variable('b', np.zeros((1, output_shape[0]), dtype=theano.config.floatX), broadcastable=(True,False)) # 设置变量b
            # 这一层的输出为 x·w + b
...
class Standardizer(Model):
    def __init__(self, dim, eps=1e-6, init_count=0, init_mean=0., init_meansq=1.):
        ...
        Args:
            ...
            eps: small constant to add to denominators to prevent division by 0 # 防止出现除0情况
        ...
        self.get_mean = self._mean_1_D.get_value # 期望
        self.get_stdev = theano.function([], self._stdev_1_D[0,:]) #标准差 TODO: return with shape (1,D)
    ...
    def standardize_expr(self, x_B_D): #将数据标准化
