    ...
    def __init__(self, obsfeat_space, action_space, hidden_spec, max_kl, adam_lr, adam_steps, ent_reg_weight, enable_inputnorm, include_time, time_scale, favor_zero_expert_reward, varscope_name):
        ...
        with nn.variable_scope(varscope_name) as self.__varscope:
            ...
            if isinstance(self.action_space, ContinuousSpace):
                # For a continuous action space, map observation-action pairs to a real number (reward) 把s-a对映射变为一个真实的数，也就是返回的奖励值
        ...
        # 判断返回的reward是否需要截断，如mountain car 在达到山顶时就需要被截断
        if self.favor_zero_expert_reward:
            # 0 for expert-like states, goes to -inf for non-expert-like states 0表示专家状态，-inf表示非专家状态
            # compatible with envs with traj cutoffs for good (expert-like) behavior
            # e.g. mountain car, which gets cut off when the car reaches the destination
            rewards_B = thutil.logsigmoid(scores_B) # 当scores_B越接近0时，概率越高，越像专家状态
        else:
            # 0 for non-expert-like states, goes to +inf for expert-like states
            # compatible with envs with traj cutoffs for bad (non-expert-like) behavior
            # e.g. walking simulations that get cut off when the robot falls over
            rewards_B = -tensor.log(1.-tensor.nnet.sigmoid(scores_B)) # 越接近零的，概率越低，越不像专家状态
        if self.include_time:
        ...
        # 伯努利交叉熵
        ent_B = thutil.logit_bernoulli_entropy(scores_B) # −𝑝log𝑝−(1−𝑝)log(1−𝑝)
        loss = ((losses_B - self.ent_reg_weight*ent_B)*weights_B).sum(axis=0) # 定义损失函数
        lossgrad_P = thutil.flatgrad(loss, param_vars) # 求损失函数的梯度
        ...
        if self.include_time:
            ...
            adamstep_without_time = thutil.function(
                ...
                updates=thutil.adam(loss, param_vars, lr=adam_lr)) # 使用adam优化器
...
class LinearReward(object):
    ...
    def _featurize(self, obsfeat_B_Do, a_B_Da, t_B):
        ...
        if isinstance(self.action_space, ContinuousSpace):
            ...
            action_inds = [np.flatnonzero(a_B_Da[:,0] == a) for a in range(self.action_space.size)]
...
class ImitationOptimizer(object):
    ...
    def step(self):
        with util.Timer() as t_all:
            # Sample trajectories using current policy 使用当前策略运行，进行轨迹采样
            # print 'Sampling'
            ...
            # print 'Computing advantages' # 计算优势函数
            with util.Timer() as t_adv:
                # Compute observation features for reward input 为判别网络计算环境特征向量
                ...
                # If we're regularizing the policy, add negative log probabilities to the rewards 添加负对数概率到reward中
                # Intuitively, the policy gets a bonus for being less certain of its actions 该政策因其行动的确定性较低而获得奖金。
                ...
                # Compute advantages using these rewards 利用当前策略得到的回报计算优势值
            ...
            # Take a step
            # print 'Fitting policy' 更新G，利用TRPO
            with util.Timer() as t_step:
                params0_P = self.policy.get_params()
                # self.policy = GuassianPolicy
