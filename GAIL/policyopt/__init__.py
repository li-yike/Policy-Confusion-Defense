...
class MDP(object):
    ...
    def sim_single(self, policy_fn, obsfeat_fn, max_traj_len, init_state=None):
        ...
        for _ in range(max_traj_len):
            ...
            a, adist = policy_fn(obsfeat[-1]) # a 选择的动作 adist动作概率分布
            actions.append(a)
            actiondists.append(adist)
            rewards.append(sim.step(a[0,:]))
            if sim.done: break # 达到终态结束
    ...
    def sim_mp(self, policy_fn, obsfeat_fn, cfg, maxtasksperchild=200):
        '''
        Multiprocessed simulation 多线性进行模拟
        Not thread safe! But why would you want this to be thread safe anyway?
        '''
        num_processes = cfg.batch_size if cfg.batch_size is not None else multiprocessing.cpu_count()//2 # 判断是否可以多线程
...
def _rollout():
    try:
        ...
        np.random.seed(int(binascii.hexlify(os.urandom(4)).decode(), 16))
        global _global_sim_info
