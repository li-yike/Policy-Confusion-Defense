...
def ngstep(x0, obj0, objgrad0, obj_and_kl_func, hvpx0_func, max_kl, damping, max_cg_iter, enable_bt):
    ...
    Args:
        ...
        max_cg_iter: max conjugate gradient iterations for solving for natural gradient step 求解自然梯度步骤的最大共轭梯度迭代
    '''

    assert x0.ndim == 1 and x0.shape == objgrad0.shape

    # Solve for step direction 求解更新关键
...
def make_ngstep_func(model, compute_obj_kl, compute_obj_kl_with_grad, compute_kl_hvp):
    ...
    def wrapper(feed, max_kl, damping, subsample_hvp_frac=.1, grad_stop_tol=1e-6, max_cg_iter=10, enable_bt=True):
        ...
        gnorm = util.maxnorm(objgrad0) # 梯度的最大值
        ...
        # Data subsampling for Hessian-vector products # 为了Hessian-vector products 二次抽样
        ...
        params1, num_bt_steps = ngstep(
            x0=params0, # 需要更新的参数
            obj0=-obj0, # 更新依据
            objgrad0=-objgrad0, # 更新依据
            obj_and_kl_func=obj_and_kl_func, # obj_kl的计算方法
            hvpx0_func=hvpx0_func, # Hessian-vector products计算方法
            max_kl=max_kl, # 可以接受的最大kl
            damping=damping,
            max_cg_iter=max_cg_iter,
            enable_bt=enable_bt)
        model.set_params(params1) # 设置模型参数
