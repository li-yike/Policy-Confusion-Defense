...
def load_dataset(filename, limit_trajs, data_subsamp_freq):
    ...
    print("expert return: ",exr_B_T.sum(axis=1))
...
def main():
    ...
    parser.add_argument('--limit_trajs', type=int, required=True) # 每个batch的轨迹数量
    parser.add_argument('--data_subsamp_freq', type=int, required=True) # 二次采样频率，即每20个采样至少一次
    # MDP options
    parser.add_argument('--env_name', type=str, required=True)
    parser.add_argument('--max_traj_len', type=int, default=None) # 每个轨迹最多动作次数
    # Policy architecture
    parser.add_argument('--policy_hidden_spec', type=str, default=SIMPLE_ARCHITECTURE) # 策略隐藏层
    parser.add_argument('--tiny_policy', action='store_true') # 是否是最简单的策略
    parser.add_argument('--obsnorm_mode', choices=OBSNORM_MODES, default='expertdata')
    # Behavioral cloning optimizer
    parser.add_argument('--bclone_lr', type=float, default=1e-3) # 克隆学习的学习率
    parser.add_argument('--bclone_batch_size', type=int, default=128) # 克隆学习的batch数量
    ...
    # Imitation optimizer
    parser.add_argument('--discount', type=float, default=.995) # 折扣因子
    parser.add_argument('--lam', type=float, default=.97)
    parser.add_argument('--max_iter', type=int, default=1000000) # 最大迭代次数######### 1000000
    parser.add_argument('--policy_max_kl', type=float, default=.01) # 策略可以接受的最大kl散度
    ...
    parser.add_argument('--reward_type', type=str, default='nn') # 反馈函数的类型
    # parser.add_argument('--linear_reward_bin_features', type=int, default=0)
    parser.add_argument('--reward_max_kl', type=float, default=.01) # 回报的最大kl散度
    parser.add_argument('--reward_lr', type=float, default=.01) # 回报的学习率
    parser.add_argument('--reward_steps', type=int, default=1) # 回报的更新步频
    ...
    parser.add_argument('--min_total_sa', type=int, default=10000)######### 50000
    parser.add_argument('--favor_zero_expert_reward', type=int, default=0) # 是否以0为专家策略
    # Saving stuff
    ...
    if isinstance(mdp.action_space, policyopt.ContinuousSpace):
        policy_cfg = rl.GaussianPolicyConfig(
            hidden_spec=args.policy_hidden_spec, # 定义生成网络的隐藏层
            min_stdev=0., # 最小标准差
            init_logstdev=0., # 初始log标准差
            enable_obsnorm=enable_obsnorm) # 环境是否需要标准化
    ...
    # Load expert data 载入专家轨迹，其中的环境、动作、二次采样的频率
    exobs_Bstacked_Do, exa_Bstacked_Da, ext_Bstacked = load_dataset(
        args.data, args.limit_trajs, args.data_subsamp_freq)
    # exobs_Bstacked_Do,[200,11],一共200条每个状态有11个参数
    # exa_Bstacked_Da,[200,3],一共200个动作，每个动作3个参数
    ...
    if args.mode == 'bclone':
        ...
        if args.reward_type == 'nn':
            # TransitionClassifier是分类器，即对输入的样本进行分类辨别，输出的参数当作reward，即论文中第一步的初始化判别器的参数
