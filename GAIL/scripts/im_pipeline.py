...
import json
import sys
sys.path.append("/usr/local/WorkSpace/Imitation_OpenAI/")
import h5py
import numpy as np
import yaml
...
def phase0_sampletrajs(spec, specfilename):
    ...
    for task in spec['tasks']:
        ...
        print('ret: {} +/- {}'.format(returns.mean(), returns.std())) # 回报总数
        print('avgr: {}'.format(avgr)) # 回报均值
        print('len: {} +/- {}'.format(lengths.mean(), lengths.std())) # 平均运行次数 +/- 方差
        print('ent: {}'.format(ent)) # 熵
...
def main():
    ...
    with open(args.spec, 'r') as f:
        spec = yaml.load(f,Loader)    # 之前改为spec = yaml.load(f, Loader=LineLoader)

    phases[args.phase](spec, args.spec) # phases[0_sampletrajs](spec) => phase0_sampletrajs(spec)
