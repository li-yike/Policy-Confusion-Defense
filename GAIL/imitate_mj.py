...
def main():
    ...
    parser.add_argument('--limit_trajs', type=int, required=True) # batch size
    ...
    parser.add_argument('--', type=int, default=1000000) # max iterations
    ...
    parser.add_argument('--reward_type', type=str, default='nn') # type of reward to use, nn/l2ball/simplex
    ...
    # Check whether the current policy is continuous
    ...
    if args.mode == 'bclone':
        ...
        # Nonlinear reward calculation
        if args.reward_type == 'nn':
            # TransitionClassifier is a classifier that discriminates input samples, and its output parameters are used as rewards (i.e., the initialized discriminator parameters in Step 1 of the paper)
        ...
        vf = None if bool(args.no_vf) else rl.ValueFunc(
            ...
            damping=args.vf_cg_damping, # Conjugate gradient damping
            time_scale=1./mdp.env_spec.max_episode_steps,
            varscope_name='ValueFunc')
        # Define imitation learning optimizer
    ...
    if args.obsnorm_mode == 'expertdata':
        # Update environment variables within the policy with regularization
