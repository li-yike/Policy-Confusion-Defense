...
if __name__ == "__main__":
    ...
    # logs
    exp_id = f"/root/f-IRL/logs/{env_name}/exp-{num_expert_trajs}/{v['obj']}"  # task/obj/date structure
    # exp_id = 'debug'
    ...
    os.system(f'cp /root/f-IRL/baselines/main_samples.py {log_folder}')
    os.system(f'cp /root/f-IRL/baselines/adv_smm.py {log_folder}')
    ...
    if v['obj'] != 'airl':
        load_path = f'/root/f-IRL/expert_data/states/{env_name}.pt' 
    else:
        load_path = f'/root/f-IRL/expert_data/states/{env_name}_airl.pt' 
    expert_trajs = torch.load(load_path).numpy()[:, :, state_indices]
    print("num_expert_trajs=", num_expert_trajs)
    ...
    if v['obj'] != 'airl':
        ...
        print("expert_trajs.shape=", expert_trajs.shape)
    ...
    expert_action_trajs = torch.load(f'/root/f-IRL/expert_data/actions/{env_name}_airl.pt').numpy()
