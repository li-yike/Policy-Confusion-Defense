...
if __name__ == "__main__":
    ...
    # logs
    exp_id = f"f-IRL/logs/{env_name}/exp-{num_expert_trajs}/{v['obj']}" # task/obj/date structure
    # exp_id = 'debug'
    ...
    expert_trajs = torch.load(f'f-IRL/expert_data/states/{env_name}.pt').numpy()[:, :, state_indices]
    ...
    for itr in range(v['irl']['n_itrs']):
        ...
        if real_return_det > max_real_return_det and real_return_sto > max_real_return_sto:
            ...
            torch.save(reward_func.state_dict(), os.path.join(logger.get_dir(), 
                    f"f-IRL/model/reward_model_itr{itr}_det{max_real_return_det:.0f}_sto{max_real_return_sto:.0f}.pkl"))
